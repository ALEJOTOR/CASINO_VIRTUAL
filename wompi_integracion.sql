-- ============================================================
-- wompi_integracion.sql
-- Descripcion: Tabla datos bancarios, extension PKG_USUARIOS
-- Esquema: SEBAS_CASINO, Tablespace: CASINO_VIRTUAL
-- ============================================================

-- ============================================================
-- 1. DATOS_BANCARIOS_USUARIO
-- ============================================================
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_count FROM user_tables WHERE table_name = 'DATOS_BANCARIOS_USUARIO';
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE TABLE datos_bancarios_usuario (
            id_datos_bancarios NUMBER        PRIMARY KEY,
            id_usuario         NUMBER        NOT NULL,
            banco_id           VARCHAR2(10)  NOT NULL,
            ...
            CONSTRAINT fk_db_usr FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario)
...
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_count FROM user_sequences WHERE sequence_name = 'SEQ_DATOS_BANCARIOS';
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'CREATE SEQUENCE seq_datos_bancarios MINVALUE 1 START WITH 1 INCREMENT BY 1 CACHE 20';
    END IF;
END;
/

-- ============================================================
-- 2. VW_DATOS_BANCARIOS
-- ============================================================
CREATE OR REPLACE VIEW vw_datos_bancarios AS
SELECT
...
-- ============================================================
-- 3. ADD PAYOUT_ID TO RETIROS
-- ============================================================
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_count FROM user_tab_columns
    WHERE table_name = 'RETIROS' AND column_name = 'PAYOUT_ID';
    IF v_count = 0 THEN
        EXECUTE IMMEDIATE 'ALTER TABLE retiros ADD (payout_id VARCHAR2(100))';
    END IF;
END;
/

-- ============================================================
-- 4. EXTEND PKG_USUARIOS SPEC
-- ============================================================
...
PROMPT 'wompi_integracion.sql ejecutado correctamente.'
